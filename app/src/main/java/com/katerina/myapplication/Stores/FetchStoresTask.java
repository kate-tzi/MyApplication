...
public class FetchStoresTask extends AsyncTask<String,Void,ArrayList<Store>> {
    ...
    private ArrayList<Store> getMerchantsFromJson(String storesJsonStr) throws JSONException {
        ArrayList<Store>  stores = new ArrayList<>();
        try{
            JSONArray storesArray = new JSONArray(storesJsonStr);
            ...
            for(int i=0; i<storesArray.length(); i++){
                ...
                JSONObject jsonContact = jsonStore.getJSONObject("contactPoint");
                String storeAddress = jsonContact.getString("streetAddress");

                JSONObject jsonRating = jsonStore.getJSONObject("aggregateRating");
                int storeRating = jsonRating.getInt("ratingValue");

                stores.add(new Store(storeId, storeName, storeCategory, storeAddress, storeRating));
            }

            Log.d(LOG_TAG, "Stores Fetching Complete. " + stores.size() + "stores inserted");
            return  stores;
    ...
    protected ArrayList<Store> doInBackground(String... params) {
        ...
        try {
            ...
            return  getMerchantsFromJson(storesJsonStr);
        } catch (IOException e) {
